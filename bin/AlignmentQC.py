...
def ColocCoef(img1, img2):
    ...
    X_bar = np.average(X)
    Y_bar = np.average(Y)
    Pearson = np.sum((X - X_bar) * (Y - Y_bar)) / (
...
def GetImgs(filepath, Ncyc, DapiCh):
    with TiffFile(filepath) as tif:
        imgs = tif.asarray()
    shp = imgs.shape
    Imgs = []
    # find axis with >1 size
    if len(shp) <= 3:
        channels = np.arange(DapiCh, imgs.shape[0], int(imgs.shape[0] / Ncyc)).tolist()
        for i in range(shp[0]):
            ni = channels.count(i)
            if ni > 0:
                Imgs.append(imgs[i, :, :].astype("uint16"))
    else:
        shps_larger1 = [i for i in range(len(shp)) if shp[i] > 1]
        axis = shps_larger1[0]  # we assume only one axis is containing frames
        channels = np.arange(
            DapiCh, imgs.shape[axis], int(imgs.shape[axis] / Ncyc)
        ).tolist()
        for i in range(shp[axis]):
            ni = channels.count(i)
            if ni > 0:
                frame = imgs.take(indices=i, axis=axis)[
                    0, 0, :, :
                ]  # here number of zeros corresponds to 5d array!
                Imgs.append(frame.astype("uint16"))
    del imgs
    return Imgs
...
def TileAnalysis(img1, img2, TileSize):
    ResList = []
    PearsonC = np.zeros(
        int(np.floor(img1.shape[0] / TileSize))
        * int(np.floor(img1.shape[1] / TileSize))
    )
    for i in range(int(np.floor(img1.shape[0] / TileSize))):
        for j in range(int(np.floor(img1.shape[1] / TileSize))):
            subimg1 = img1[
                i * TileSize : (i + 1) * TileSize, j * TileSize : (j + 1) * TileSize
            ]
            subimg2 = img2[
                i * TileSize : (i + 1) * TileSize, j * TileSize : (j + 1) * TileSize
            ]
...
def GenerateTileImage(img1, img2, ListPosPear, TileSize):
    ...
    IMG = np.zeros((3, img1.shape[0], img1.shape[1]))
    IMG[1, :, :] = img1
    IMG[2, :, :] = img2
    MaxIntensity = 65536
    ...
    for posP in ListPosPear:
        if not np.isnan(posP[2]):
            x0 = int(posP[0] - TileSize / 2 + border)
            x1 = int(posP[0] + TileSize / 2 - border)
            y0 = int(posP[1] - TileSize / 2 + border)
            y1 = int(posP[1] + TileSize / 2 - border)
                ...
                if os.path.isfile(output_csv):
                    ...
                    with open(output_csv, "w") as f_object:
                        ...
                        writer_object.writerow(
                            ["Cycles_compared", "Pearson", "Manders"]
                        )
                        writer_object.writerow(Row)
                        f_object.close()
...
def read_tiff_channel(path, channel):
    ...
    try:
        ...
        raise ValueError(f"Selected channel ({channel}) out of range: {path}") from None
...
def compose_block(base, img, brightness, in_range):
    img = skimage.exposure.rescale_intensity(img, in_range=in_range, out_range=float)
...
def show_progress(futures, desc=None):
    n = len(futures)
    progress = tqdm.tqdm(concurrent.futures.as_completed(futures), total=n, desc=desc)
    for _ in progress:
        pass
...
def build_panel2(
    ...
    p1 = np.dstack(
        np.meshgrid(range(shifts.shape[0]), range(shifts.shape[1]), indexing="ij")
    )
    ...
    )  # just simply linear fit with minimizing square displacements
    ...
    (a, b), (
        c,
        d,
    ) = lr.coef_  # it is like transf matrix between stilled and moved set of tiles
    # T = [[a b], [c d]
    ...
    line_csv = [
        pairnames,
        np.mean(abs(shifts), axis=(0, 1)),
        np.median(abs(shifts), axis=(0, 1)),
        [scale_y, scale_x],
        shear,
        rotation,
    ]
    ...
    # print("    colorizing")
    heatmap_small = (
        colorize2(distance, distance_range) * bmask[..., None]
    )  # colorization on level of tiles
    hs = w // out_scale
    panel = heatmap_small.repeat(hs, axis=1).repeat(
        hs, axis=0
    )  # extend colorization on whole (downscaled) iamge
...
def block_reduce_nopad(image, block_size, func=np.sum, cval=0):
    ...
    if (np.array(image.shape) % block_size).any():
        raise ValueError("`image.shape` must be an integer multiple of `block_size`.")
...
def main_OptFlow(Imgs, output_folder, block_threshold, ga_downscale, bsize):
    ...
    output_brightness_scale = (
        1  # Multiplier for reference image intensity in the output image
    )
    ...
    distance_range = [
        0,
        1000,
    ]  # Specify the range of shift distances for visualization Usually it is between 0 and the maximum expected shift value from all images"
    ...
    for i in range(len(Imgs) - 1):
        for j in range(i + 1, len(Imgs)):
            ...
            bmax = skimage.measure.block_reduce(
                c1, (bsize, bsize), np.max
            )  # Downsample image by    applying function function 'np.max' to local blocks (but other functions like np.median can be used)
            bmask = bmax > block_threshold  # here we filter out low intensity areas
            bmask = skimage.morphology.remove_small_objects(
                bmask, min_size=area_threshold
            )
            ...
            output_path = (
                output_folder + "/OptFlow_Cyc" + str(i) + "_vs_Cyc" + str(j) + ".tif"
            )
            add_vector_plot(
                panel,
                shifts / (2 * ga_downscale),
                pos_shifts / (2 * ga_downscale),
                output_path,
            )
