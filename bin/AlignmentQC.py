...
def GetImgs(filepath, channels):
    ...
    if channels:
        try:
            ...
            img = np.zeros(
                (len(channels), memmap_volume.shape[3], memmap_volume.shape[4]),
                dtype="uint16",
            )
            i = 0
            for nch in channels:
            ...
            # print(z)
            img = np.zeros(
                (len(channels), immg.shape[0], immg.shape[1]), dtype="uint16"
            )
            i = 0
            del immg
    ...
    for i in range(len(Imgs) - 1):
        for j in range(i + 1, len(Imgs)):
            ...
            if np.sum(bmask) > 3:
                ...
                print(
                    "Optical flow for Cycle "
                    + str(i)
                    + " vs Cycle "
                    + str(j)
                    + " doesnt work due to high OF_BlockThresh"
                )
            # print(panel)
            # print(dist)
            # print(shifts)
